...
def handle_event(event, context):
    ...
    if not appointment_id or not appointment_time_str or not reminder_type:
        logger.error(
            "Missing appointment_id, appointment_time or reminder_type in event detail."
        )
        return {
            "statusCode": 400,
            "body": json.dumps(
                {"error": "Missing appointment_id, appointment_time or reminder_type"}
            ),
        }
    ...
    try:
        response = table.get_item(
            Key={"appointment_id": appointment_id, "time_stamp": appointment_time_str}
        )
    except Exception as e:
    ...
    if not appointment:
        logger.error(
            f"Appointment not found for ID: {appointment_id} and time_stamp: {appointment_time_str}"
        )
        return {
            "statusCode": 404,
    ...
    return {
        "statusCode": 200,
        "body": json.dumps(
            {"message": f"{reminder_type} reminder email sent successfully"}
        ),
    }
